...
// Reject Birthday Wish
export const rejectWishController = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Create a notification for the user
        const notificationMessage = "Your birthday wish has been rejected.";
        const notification = new Notification({
            postedBy: updatedWish.postedBy,
            message: notificationMessage,
            templateType: updatedWish.templateType, // Assuming templateType is stored in birthdayModel
        ...
        res.status(200).send({
            success: true,
            message: "Birthday wish rejected successfully",
            data: {
                notification: notification,
            },
        });
    } catch (error) {
        console.error("Error rejecting birthday wish:", error);
        res.status(500).send({
            success: false,
            message: "Internal server error",
        });
